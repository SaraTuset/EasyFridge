...
type
    // Estructura del nodo
    nodo = record
        info: TProducto; // Información almacenada en el nodo
...
function first(c: tCola): TProducto;
function last(c: tCola): TProducto;
procedure enqueue(var c: tCola; x: TProducto);
...
begin
    ...
    while aux <> nil do
    begin
        s := s + getNombre(aux^.info); // Agregar la información del nodo a la cadena
